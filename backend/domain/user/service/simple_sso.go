...
func GetSimpleUserInfo(userConfig *userConfig.Config, platform, key, payload string) (*SsoUserInfo, error) {
	// 检测平台是否在配置中
	ssoConfig, err := GetSimpleSSOConfigByPlatform(platform, userConfig)
	if err != nil {
		return nil, err
	}

	// 通过私钥解密出aes key 和 iv
	aesKeyIv, err := RsaDecryptAesKeyIV(ssoConfig.PrivateKey, key)
	if err != nil {
...
func GetSimpleSSOConfigByPlatform(platform string, userConfig *userConfig.Config) (*userConfig.SimpleSSOConfig, error) {
	// 确保平台标识符不为空
	if platform == "" {
	...
	for _, ssoConfig := range userConfig.SimpleSSO {
